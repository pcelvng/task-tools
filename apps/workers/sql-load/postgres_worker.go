...
func (o *options) newPostgres(info string) task.Worker {
	w := &workerPostgres{
		options: *o,
		Meta:    task.NewMeta(),
		ds:      NewDataSet(),
...
func (w *workerPostgres) DoTask(ctx context.Context) (task.Result, string) {
...
func (w *workerPostgres) QuerySchema() (err error) {
 ...
 FROM information_schema.columns WHERE table_schema = '%s' AND table_name = '%s'`
	...
	if w.dbDriver == "postgres" {
		...
		return fmt.Errorf("wrong db type, not postgres worker (%s)", w.dbDriver)
	}
